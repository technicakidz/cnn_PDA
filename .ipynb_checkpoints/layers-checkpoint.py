...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    sample_mean = xp.mean(x,axis=0)
    sample_var = xp.var(x,axis=0)

    num = x - sample_mean
    denom = xp.sqrt(sample_var + eps)
    xnorm = num / denom
    ...
    xnorm = (x - running_mean) / xp.sqrt(running_var + eps)
    out = gamma*xnorm + beta
...
def batchnorm_backward(dout, cache):
  ...
  dgamma = xp.sum(cache['xnorm'] * dout, 0)
  dbeta = xp.sum(dout, 0)
  ...
  dnorm = dout * gamma
  dnorm2 = xp.sum(-(num * dnorm) / np.square(denom), 0)
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  out = xp.zeros((N, F, Hp, Wp))
  xpad = xp.pad(x, [(0,0), (0,0), (P,P), (P,P)], 'constant')
...
def svm_loss(x, y):
  ...
  """
  N = x.shape[0]
  correct_class_scores = x[xp.arange(N), y]
  margins = xp.maximum(0, x - correct_class_scores[:, np.newaxis] + 1.0)
  margins[xp.arange(N), y] = 0
  loss = xp.sum(margins) / N
  num_pos = xp.sum(margins > 0, axis=1)
  dx = xp.zeros_like(x)
  dx[margins > 0] = 1
  dx[xp.arange(N), y] -= num_pos
  dx /= N
  return loss, dx
...
def softmax_loss(x, y):
  ...
  N = x.shape[0]
  loss = -xp.sum(xp.log(probs[xp.arange(N), y])) / N
  dx = probs.copy()
  dx[xp.arange(N), y] -= 1
  dx /= N
  return loss, dx
