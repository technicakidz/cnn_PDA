...
def jpeg2matrix(path):
	...
	im = Image.open(path)
	m = xp.asarray(im.convert('RGB')) #np.asarray:引数に配列コピーせずそのまま返す
	m = xp.transpose(m, (2,0,1)).astype("float") #np.transpose(a,axes=None):転置，反転する軸を引数で指定することも可
...
def get_data(nsubj=6, type="gallery"):
	...
	if type=="probe":
		path = "/Users/yoshidatakayuki/Desktop/研究データ/Data/PDA/Probe"
	else:
		path = "/Users/yoshidatakayuki/Desktop/研究データ/Data/PDA/gallery"
	...
	lim = xp.argmax(sub_id>nsubj) #np.argmax:引数に与えられた配列の最大インデックスを取得
	files = files[:lim]
	...
	X = xp.zeros((nfiles, 3, 256, 256))　#np.zeros:要素がすべて0である0行列を新しく生成する関数
	y = xp.zeros(nfiles)
	...
	# Normalize X
	X -= xp.mean(X, axis=0)　#np.mean:配列の平均
...
def get_data_sametaum(nsubj=6, type="gallery", tau=1, m=1):
	...
	if type=="probe":
		path = "/Users/yoshidatakayuki/Desktop/研究データ/Data/PDA/prove"
	else:
		path = "/Users/yoshidatakayuki/Desktop/研究データData/PDA/gallery"
	...
	# get subjects for each file
	sub_id = [x.split("_")[3] for x in files]
	sub_id = xp.asarray(map(float, sub_id)) #numpy

	# keep only the number of subjects needed
	lim = xp.argmax(sub_id>nsubj) #numpy
	files = files[:lim]
	#print(files)
	...
	# Initialize outputs
	X = xp.zeros((nfiles, 3, 256, 256)) #numpy
	y = xp.zeros(nfiles) #numpy
